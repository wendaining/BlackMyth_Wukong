...
AWukongCharacter::AWukongCharacter()
{
    ...
    // 骨骼网格、动画蓝图等资源应该在蓝图中设置，而不是硬编码在C++中
    // 这样可以方便地切换不同的角色模型和动画
    // 在 BP_Wukong_Player 蓝图中设置 SkeletalMesh 和 AnimInstance
    
    // 所有动画资源（Montages、Sequences）也应该在蓝图中设置
    // 这些属性已经在头文件中声明为 UPROPERTY(EditAnywhere)
    // 因此可以在蓝图的 Class Defaults 中配置
