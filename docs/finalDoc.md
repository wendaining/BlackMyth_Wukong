...
一个基于 Unreal Engine 的第三人称动作玩法原型项目，围绕战斗体验、敌人与 Boss 行为、场景与交互系统、物品与状态效果、土地庙交互与传送、商店与金币、存档读档等核心要素迭代开发。文档按功能模块梳理当前实现与技术细节。本文将功能与技术要点分区说明，并在最后提供简要的项目分工表。
...
| githubID    | 贡献度 | 负责内容                                                     |
| ----------- | ------ | ------------------------------------------------------------ |
| wendaining  | 25%    | 战斗系统与技能（定身术、立棍 AOE）、音效系统与角色行为修复、代码审查与编码规范统一、字体与 UI 优化、NPC 对话期间交互约束、相机/移动问题修复 |
| Palind-Rome | 25%    | Boss（二郎神）全套：蓝图与招式、闪避与哮天犬、开场动画与摄像机、战区空气墙与触发器、敌人武器与远程/斧头/剑兵扩展、对话与演出兼容性修复 |
| onnisama    | 25%    | Temple 系统：传送（指定点/地图分布）、复活点与重生、回复交互、Temple 面板与 Q 键提示、菜单 UI 优化与键位调整、保存读档与交互稳定性修复 |
| Aphrody-Dy  | 25%    | 商店与金币：掉落/拾取/HUD、商品管理器与条目控件、背包/物品系统（药瓶、Buff UI、装备与键位冲突修正）、状态效果系统（中毒/减速）与 HUD 细节完善 |
